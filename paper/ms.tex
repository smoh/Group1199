...
% packages
\definecolor{cbblue}{HTML}{0D47A1}
...
\begin{abstract}
  We report and discuss the discovery of a significant difference in chemical
  abundances of a comoving pair of bright solar-type
  stars, HD~240430 and HD~240429.
  The two stars have an estimated 3D separation of $\approx 0.6$~pc
  ($\approx 0.01$~pc projected) at a distance of
  ...
  ($\Tcondens>1200$~K) elements by $\approx 0.2$~dex and a marginal
  enhancement of (moderately) volatile elements ($\Tcondens<1200$~K; \elem{C},
  \elem{N}, \elem{O}, \elem{Na}, and \elem{Mn}).
  This is the largest metallicity difference found in a wide binary pair yet.
  Additionally, HD~240430 shows an anomalously high surface lithium abundance
  ($A(\elem{Li})=2.75$), higher than its cooler companion by $0.5$~dex.
  ...
  refractory elements as well as lithium in its surface and convective
  envelope.
\end{abstract}
...
\begin{figure}[htpb]
  ...
  \caption{Abundance difference in this pair and other twin-like
    ...
    Thus, the narrower range of $\Delta\feh$ is by construction.
...
For a 2~\msun\ binary system, the Jacobi radius in the Solar neighborhood is
...
residuals for almost all metal line features, largely dominated by \elem{Fe}.
...
\emph{end-of-mission} Gaia catalog with TODO model, and 
looked for a chance pair of solar-mass ($0.9~M_\odot< M < 1.1~M_\odot$) primary
stars ignoring any companions to the primary.
...
Another possibility that two stars unrelated at birth may end up in a binary
system is via a binary-single scattering event that results in an exchange of
binary members.
...
\software{
  ...
  \texttt{corner.py} (\citealt{corner}),
