...
{\bf \noindent
  ...
  chemical abundances that strongly suggests accretion of a massive rocky
...
validity of the measured abundance differences (\figname~\ref{fig:spec1} and
\ref{fig:spec2}).

% novelty
The observed difference in metallicity and abundances are genuinely rare and
unexpected.
None of the other four twin-like ($\Delta T_\mathrm{eff} \lesssim 100$~K), wide-binary pairs examined using spectra of similar quality and the same
...
Sorting the abundance differences by condensation temperature ($T_C$) of the
...
scattering\cite{TODO} or encounters with a field star\cite{Malmberg:2011aa} can
...
elemental composition of the pre-solar nebula. \todo{Check end of last sentence}
...
to be ordinary, the accretion of $20$~\mearth\ by \bizarreone\ is consistent with a $1.6$~dex
...
within the last $\sim 1$~Gyr remains unclear.
...
have propagated inward through planet-planet scattering, leading to the accretion of inner rocky planets\cite{Zakamska:2004aa,Malmberg:2011aa}.
...
search programs, however, outer eccentric planets are potentially detectable with future data releases of the \gaia\ mission.
...
\begin{figure}[htpb]
  ...
  \caption{Same as \figname~\ref{fig:spec1}
    ...
    line that was studied separately in Myles \etal (in prep.).
...
normal (Gaussian) distribution (see also appendices of
\citealt{2017AJ....153..257O}).
...
of rocky, Lithium-rich material.
...
explain the chemical differences between the two stars is that there was
...
Here we estimate how much added rocky material is needed to explain an
