...
HD~240429 and HD~240430, with unusual chemical abundance and differential abundance patterns
...
We compared the measured $\log(g)$ and $T_\mathrm{eff}$ to \project{MIST} isochrones at two
different $\feh$ of $0$ and $0.2$, corresponding to the $\feh$ of \sunanalog\ and \bizarreone.
The ages most consistent with $\log(g)$ and $t_\mathrm{eff}$ for
\sunanalog\ and \bizarreone\ using the isochrones of corresponding metallicities
is $\approx 4$~Gyr and $\approx 5.6$~Gyr respectively, based on $\chi^2$ values.
This is in tension with the relatively high $\elem{Li}$ abundance in both stars.
Even for \sunanalog\, a $\elem{Li}$ abundance of 2.25 is unusually high for a sunlike star with
similar ages.

are plotted in Figure~\ref{fig:isochrones}.
For \sunanalog\ ($\feh = 0.01$), the most consistent isochrone is that for
an age of $2$~Gyr and $\feh = 0.2$.
The $\log(g)$ values of the two stars, especially of \bizarreone\, seem to suggest
that they could be at the end of the pre-main sequence phase.
However, an age of $28$~Myr for \sunanalog\ is in tension with the larger age ($800$~Myr)
inferred from its $\elem{Li}$ abundance.
Also, at such a young age, we may expect to see infrared excess from a circumstellar disk or
X-ray emission from magnetic activity. 
However, the spectral energy distributions of the two stars based on
\project{Tycho-2}, \tmass, \project{WISE} photometry (\todo{CITE}) are consistent with blackbody,
and no relatable X-ray point sources are found in the \project{ROSAT} All-Sky Survey
around the two stars,
On the other hand, the most consistent main sequence age ($2$~Gyr) also is in tension with
its relatively high $\elem{Li}$ abundance, not to mention with the $\log(g)$ of \bizarreone\
if the two stars should have the same age.
